...
const SOURCE_DIR = path.join(OMNIVOICE_DIR, 'source');
const VENV_DIR = path.join(OMNIVOICE_DIR, 'venv');
...
function findPython() {
  ...
  throw new Error('Need Python 3.10-3.12 for OmniVoice. Install Python 3.11 or set PYTHON=/path/to/python.');
}
...
function pythonSysconfig() {
  ...
  if (result.status !== 0) {
    throw new Error(result.stderr || 'Failed to inspect OmniVoice Python runtime.');
  }
...
function main() {
  if (!fs.existsSync(SOURCE_DIR)) {
    throw new Error(`Missing OmniVoice source: ${SOURCE_DIR}`);
  }
  ...
  if (!fs.existsSync(venvPython())) {
    fs.mkdirSync(OMNIVOICE_DIR, { recursive: true });
  ...
  const pipArgs = ['-m', 'pip', 'install', '--upgrade', 'pip', 'wheel', 'setuptools'];
  runVenvPython(pipArgs);

  const installArgs = ['-m', 'pip', 'install'];
  if (process.env.OMNIVOICE_PIP_EXTRA_INDEX_URL) {
    installArgs.push('--extra-index-url', process.env.OMNIVOICE_PIP_EXTRA_INDEX_URL);
  }
  if (process.env.OMNIVOICE_EDITABLE === '1') {
    installArgs.push('-e');
  }
  installArgs.push(SOURCE_DIR);
  runVenvPython(installArgs);
  runVenvPython(['-c', 'import omnivoice; print("OmniVoice import ok")']);
  ensureWindowsPortablePython({
    rootDir: ROOT_DIR,
  ...
  if (process.platform === 'win32') {
    runPackagedPython(['-c', 'import omnivoice; print("OmniVoice portable Python import ok")']);
  }
  makeDarwinVenvPortable();

  console.log(`OmniVoice runtime ready: ${venvPython()}`);
}
